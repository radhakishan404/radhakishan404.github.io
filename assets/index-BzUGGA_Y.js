...
`,r=r.substring(A.length+1),h=!0),!h){let Y=this.rules.other.nextBulletRegex(z),K=this.rules.other.hrRegex(z),D=this.rules.other.fencesBeginRegex(z),H=this.rules.other.headingBeginRegex(z),_=this.rules.other.htmlBeginRegex(z),te=this.rules.other.blockquoteBeginRegex(z);for(;r;){let L=r.split(`
`,1)[0],q;if(A=L,this.options.pedantic?(A=A.replace(this.rules.other.listReplaceNesting,"  "),q=A):q=A.replace(this.rules.other.tabCharGlobal,"    "),D.test(A)||H.test(A)||_.test(A)||te.test(A)||Y.test(A)||K.test(A))break;if(q.search(this.rules.other.nonSpaceChar)>=z||!A.trim())b+=`
