...
<meta name="github" content="https://github.com/radhakishan404/hallmark-prompt-vault">
...
${v}`:v,c=c?`${c}
${b}`:b;let S=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(b,m,!0),this.lexer.state.top=S,s.length===0)break;let A=m.at(-1);if(A?.type==="code")break;if(A?.type==="blockquote"){let B=A,O=B.raw+`
`+s.join(`
`),N=this.blockquote(O);m[m.length-1]=N,l=l.substring(0,l.length-B.raw.length)+N.raw,c=c.substring(0,c.length-B.text.length)+N.text;break}else if(A?.type==="list"){let B=A,O=B.raw+`
`+s.join(`
`),N=this.list(O);m[m.length-1]=N,l=l.substring(0,l.length-A.raw.length)+N.raw,c=c.substring(0,c.length-B.raw.length)+N.raw,s=O.substring(m.at(-1).raw.length).split(`
...
`,1)[0],r[1].length),A=o.split(`
`,1)[0],B=!S.trim(),O=0;if(this.options.pedantic?(O=2,b=S.trimStart()):B?O=r[1].length+1:(O=S.search(this.rules.other.nonSpaceChar),O=O>4?1:O,b=S.slice(O),O+=r[1].length),B&&this.rules.other.blankLine.test(A)&&(v+=A+`
`,o=o.substring(A.length+1),g=!0),!g){let N=this.rules.other.nextBulletRegex(O),Q=this.rules.other.hrRegex(O),L=this.rules.other.fencesBeginRegex(O),G=this.rules.other.headingBeginRegex(O),H=this.rules.other.htmlBeginRegex(O),te=this.rules.other.blockquoteBeginRegex(O);for(;o;){let I=o.split(`
`,1)[0],W;if(A=I,this.options.pedantic?(A=A.replace(this.rules.other.listReplaceNesting,"  "),W=A):W=A.replace(this.rules.other.tabCharGlobal,"    "),L.test(A)||G.test(A)||H.test(A)||te.test(A)||N.test(A)||Q.test(A))break;if(W.search(this.rules.other.nonSpaceChar)>=O||!A.trim())b+=`
