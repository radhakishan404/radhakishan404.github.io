...
`,o=o.substring(A.length+1),g=!0),!g){let N=this.rules.other.nextBulletRegex(O),Q=this.rules.other.hrRegex(O),R=this.rules.other.fencesBeginRegex(O),G=this.rules.other.headingBeginRegex(O),H=this.rules.other.htmlBeginRegex(O),te=this.rules.other.blockquoteBeginRegex(O);for(;o;){let I=o.split(`
`,1)[0],W;if(A=I,this.options.pedantic?(A=A.replace(this.rules.other.listReplaceNesting,"  "),W=A):W=A.replace(this.rules.other.tabCharGlobal,"    "),R.test(A)||G.test(A)||H.test(A)||te.test(A)||N.test(A)||Q.test(A))break;if(W.search(this.rules.other.nonSpaceChar)>=O||!A.trim())b+=`
`+W.slice(O);else{if(B||S.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||R.test(S)||G.test(S)||Q.test(S))break;b+=`
`+A}B=!A.trim(),v+=I+`
