...
const directory = path.resolve(__dirname, "../public/articles/mcp-servers");
const source = fs.readFileSync(path.join(directory, "index.html"), "utf8");
const document = new JSDOM(source).window.document;
const url = "https://radhakishan404.is-a.dev/articles/mcp-servers/";
...
assert.equal(schema["@graph"].find((item) => item["@type"] === "TechArticle").datePublished, "2026-09-25");
...
interactive.window.document.querySelectorAll(".repo-card .owner a").forEach((link) => assert.ok(copied.includes(link.href)));
interactive.window.document.getElementById("copy-config").click();
assert.ok(copied.includes("claude mcp add"));
...
console.log("Article checks passed: 10 MCP servers, metadata, schema, anchors, assets, copy actions, and 1200x630 cover.");
