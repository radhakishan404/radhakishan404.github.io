...
const directory = path.resolve(__dirname, "../public/articles/open-source-developer-tools");
const source = fs.readFileSync(path.join(directory, "index.html"), "utf8");
const document = new JSDOM(source).window.document;
const url = "https://radhakishan404.is-a.dev/articles/open-source-developer-tools/";
...
assert.equal(schema["@graph"].find((item) => item["@type"] === "TechArticle").datePublished, "2026-09-23");
...
console.log("Article checks passed: 10 tools, metadata, schema, anchors, assets, copy actions, and 1200x630 cover.");
